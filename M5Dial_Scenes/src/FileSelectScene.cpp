...
class FileSelectScene : public Scene {
    ...
    public:
        ...
        void startArray() {
            log_println("Start array");
            if (current_key == "files")
                inFileList = true;
        }

        void startObject() { log_println("Start object"); }
    };
...
public:
    ...
    void readFromFNC() {
        ...
        // USBSerial.printf("readFromFNC( %d, %d): processMode: %d, actionMode: %d", wait_MS, timeout_MS, processMode, actionMode);
    ...
    void requestJSON() {
        ...
        for (auto const& vdir : dirVector) {
            USBSerial.printf("[%d] files: %2d, dI: %d:\"%s\"\r\n", ix++, vdir.dirEntries, vdir.displayIndex, vdir.dirName.c_str());
        }
        ix = 0;
    ...
    void onTouchRelease(m5::touch_detail_t t) { onGreenButtonPress(); }
    ...
    void showFiles(int yo) {
        ...
        for (int fx = 0; fx < 3; fx++, fdIter++) {
                    ...
                    if (selectedFile > -1) {
                        switch (fileVector[selectedFile].fileType) {
                            ...
                            case 2:
                                ...
                                if (fs > 999999.9) {
                                    snprintf(buffer, sizeof(buffer), "%.3f MB", fs / 1000000.0);
                                    fInfoB = buffer;
                                } else if (fs > 999.9) {
                                    snprintf(buffer, sizeof(buffer), "%.3f KB", fs / 1000.0);
                                    fInfoB = buffer;
                                } else {
                                    snprintf(buffer, sizeof(buffer), "%.0f bytes", fs);
                                    fInfoB = buffer;
                                }
                                break;
                        }
            ...
            if ((yo == 0) || (yo < 0 && yo + bx[fi]._yt > 45) || (yo > 0 && yo + bx[fi]._yt + bx[fi]._h < 202)) {
                auto_text(fName, bx[fi]._xt, yo + bx[fi]._yt, bx[fi]._w, (yo) ? WHITE : bx[fi]._txt, bx[fi]._f, middle_center);
            }
