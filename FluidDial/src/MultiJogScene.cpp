...
public:
    ...
    void confirm_zero_axes() {
        static char   msg[32];
        static String _confirmMsg;

        _confirmMsg = "Zero ";
        for (int axis = 0; axis < num_axes; axis++) {
            if (selected(axis)) {
                _confirmMsg += axisNumToString(axis);
            }
        }
        _confirmMsg += " ?";
        push_scene(&confirmScene, (void*)_confirmMsg.c_str());
    }
    ...
    void onTouchRelease(int x, int y) {
        log_println(String(x) + " " + String(y));
#    if 0
        int axis = which(x, y);
        if (axis < 0) {
            return;
        }
        log_println(String(axis));
...
#    endif
    ...
    static String axis_distance_string(int axis, float distance, int digits) {
        return axisNumToString(axis) + floatToString(distance, digits);
    }

    void start_mpg_jog(int delta) {
        // e.g. $J=G91F1000X-10000
        String cmd(inInches ? "$J=G91F400" : "$J=G91F10000");
        for (int axis = 0; axis < num_axes; ++axis) {
            if (selected(axis)) {
                cmd += axis_distance_string(axis, delta * distance(axis), inInches ? 3 : 2);
            }
        }
        send_line(cmd);
    }
    void start_button_jog(bool negative) {
        ...
        for (int axis = 0; axis < num_axes; ++axis) {
            if (selected(axis)) {
                ...
                cmd += axis_distance_string(axis, axis_distance, 0);
            }
        }
        send_line(cmd);
    }
